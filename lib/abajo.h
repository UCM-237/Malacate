...
extern std::atomic<float> prof;     // Contador de flancos de subida en GPIO 11


// Declaración de la función power para ejecutarse en un hilo
void abajo(int prr, int t,int nv, int &s, int& control1, bool& st1, bool& n1, bool& n2, std::atomic<int>& control, std::atomic<int>& contador, std::atomic<int>& j, std::atomic<int>& fc, std::atomic<double>& vm,std::atomic<double>& im ,std::atomic<float>& prof);
